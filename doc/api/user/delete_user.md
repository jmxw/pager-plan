...
Delete user. The [user](/doc/tech_noun/TN0202_user.md) identified by the `userId` path variable is
the target of this endpoint. The controller method currently has an empty body and performs no
action (the `UserService.delete` it would call is likewise unimplemented).
...
| --- | --- | --- |
| `userId` | Long | Identifies the user to delete. |
