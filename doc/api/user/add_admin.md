...
Add an admin user. A new [user](/doc/tech_noun/TN0202_user.md) is created with the `ADMIN` role in
the caller's [organization](/doc/tech_noun/TN0201_organization.md). The plaintext password is encoded
before being stored.

## Permission

[`ADMIN_WRITE`](/doc/permission/ADMIN_WRITE.md)
...
The role is fixed to `ADMIN` by the controller and is not read from the request body.
