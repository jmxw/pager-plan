...
Edit detail information of an article.
...
| --- | --- | --- |
| `articleId` | Long | Id of the [article](/doc/tech_noun/TN0501_article.md) to edit. |
...
| `description` | String | ✅ | New short description of the article. |
| `content` | String | ✅ | New body content of the article. |

## Response

DTO: `EmptyResponse`, returned inside the `PagerResponse` envelope as `data` — see the
[API index](/doc/api/README.md#response-envelope). The article's
[revision](/doc/tech_noun/TN0102_revision.md) is incremented and the caller becomes its
`lastAuthor`.
...
```json
{
  ...
  "content": "<p>The updated news body.</p>"
}
```
