...
| --- | --- | --- |
| `navigationId` | Long | Id of the [navigation](/doc/tech_noun/TN0601_navigation.md) node to move one position up among its siblings. |
...
[API index](/doc/api/README.md#response-envelope). The node swaps its `sequence` with the
previous sibling. When the node is already first, or is the root (no parent), nothing is changed
